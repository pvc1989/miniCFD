...
class TestLinearAdvection(unittest.TestCase):
  ...
  def test_F(self):
    ...
    # right running wave
    solver = riemann.LinearAdvection(a_const=+1)
    self.assertEqual(solver.F_on_t_axis(U_L, U_R), solver.F(U_L))
    # left running wave
    solver = riemann.LinearAdvection(a_const=-1)
    self.assertEqual(solver.F_on_t_axis(U_L, U_R), solver.F(U_R))
    # standing wave
...
class TestInviscidBurgers(unittest.TestCase):
  ...
  def test_rarefaction(self):
    ...
    self.assertEqual(solver.F_on_t_axis(U_L, U_R), 0)
