...
LX = 3;     // the length along x-axis
LY = 1;     // the length along y-axis
LZ = 2*LC;  // the length along z-axis
...
Transfinite Curve{ 1, 2, 4 } = LW/LC + 1;
Transfinite Curve{ 3, 5 } = (LX-LW)/LC + 1;
Transfinite Curve{ 6, 8 } = LH/LC + 1;
Transfinite Curve{ 7, 9, 10 } = (LY-LH)/LC + 1;
Transfinite Surface{ 1, 2, 3 };

Recombine Surface{ 1, 2, 3 };
out[] = Extrude{ 0, 0, LZ }{
  Surface{ 1, 2, 3 }; Layers{ 2 }; Recombine;
};
