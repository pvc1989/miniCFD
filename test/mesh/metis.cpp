...
TEST_F(Partitioner, PartGraphKway) {
  ...
  auto graph = mesh.GetDualGraph(n_common_nodes, index_base);
  // Partition the mesh:
  auto cell_weights = std::vector<Int>(n_cells, 1);
  for (Int j = 0; j != n_cells_y; ++j) {
    for (Int i = 0; i != n_cells_x/4; ++i) {
...
TEST_F(Partitioner, GetNodeParts) {
  ...
  int n_nodes = 10;
  std::vector<int> range{ 0, 4, 8, 11, 14 };
  std::vector<int> index{ 0, 2, 3, 1,   2, 4, 5, 3,   6, 8, 7,   8, 9, 7 };
  auto mesh = Mesh<int>(range, index, n_nodes);
  int n_parts = 3;
